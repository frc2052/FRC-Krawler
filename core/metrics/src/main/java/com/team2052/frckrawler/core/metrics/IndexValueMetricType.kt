...
abstract class IndexValueMetricType<M : ListIndexMetricWidget> : MetricType<M>() {
    private fun compiledValueToJson(compiledMap: Map<Int, com.team2052.frckrawler.core.common.Tuple2<String, Double>>, possible_values: JsonArray): JsonObject {
        val compiledValue = JsonObject()
        val values = JSON.getGson().toJsonTree(com.team2052.frckrawler.core.common.Tuple2.yieldValues(compiledMap.values).toTypedArray()).asJsonArray
    ...
    override fun compile(metric: Metric, metricValues: List<MetricValue>, weight: Float): CompiledMetricValue {
        ...
        val compiledMap = Maps.newTreeMap<Int, com.team2052.frckrawler.core.common.Tuple2<String, Double>>()
        var denominator = 0.0

        for (i in 0..possibleValues.size() - 1) {
            compiledMap.put(i, com.team2052.frckrawler.core.common.Tuple2(possibleValues.get(i).asString, 0.0))
        }
