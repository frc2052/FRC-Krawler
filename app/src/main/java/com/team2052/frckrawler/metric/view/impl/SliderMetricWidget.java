...
public class SliderMetricWidget extends MetricWidget {
    ...
    public void setMetricValue(MetricValue m) {
        min = 0;
        max = 1;
        nameText.setText(m.metric().getName());

        JsonObject range = JSON.getAsJsonObject(m.metric().getData());
        ...
        if (m.value() != null && !m.value().getAsJsonObject().get("value").isJsonNull())
            value = m.value().getAsJsonObject().get("value").getAsInt();
        else
            value = min;
    ...
    public JsonElement getData() {
        return MetricDataHelper.buildNumberMetricValue(value);
    }
}
