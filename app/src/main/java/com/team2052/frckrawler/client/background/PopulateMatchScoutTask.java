...
public class PopulateMatchScoutTask extends AsyncTask<Void, Void, Void> {
    ...
    public PopulateMatchScoutTask(ScoutMatchFragment fragment, Event event) {
        mFragment = fragment;
        this.mDaoSession = ((FRCKrawler) fragment.getActivity().getApplicationContext()).getDBSession();
    ...
    protected Void doInBackground(Void... params) {
        mMatches = mDaoSession.getDaoSession().getMatchDao().queryBuilder().orderAsc(MatchDao.Properties.Number).where(MatchDao.Properties.EventId.eq(mEvent.getId())).list();
