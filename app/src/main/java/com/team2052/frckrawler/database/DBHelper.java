...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (newVersion == 28) {
            if (oldVersion < 27) {
                db.execSQL("ALTER TABLE " + DBContract.TABLE_ROBOTS + " ADD COLUMN " + DBContract.COL_OPR);
                db.execSQL("ALTER TABLE " + DBContract.SCOUT_TABLE_ROBOTS + " ADD COLUMN " + DBContract.COL_OPR);
            }
        }
        if (oldVersion < 29) {
