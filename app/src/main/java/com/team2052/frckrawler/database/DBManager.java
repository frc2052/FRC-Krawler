...
public class DBManager {
     ...
     * Constructor
     *
     * @param _context Summary: This constructor is private because this class is a singleton. No other class should ever call
     ...
     * Method: getInstance()
     *
     * @param _context Summary: Returns an instance of the DatabaseManager. This should always be
     ...
     *
     * @param name
     * @param superuser Summary: Adds a user to the database. You can also specify if this user is
     *                  a superuser or not.
     *                  ***
     */
     ...
     * Method: removeUser
     *
     * @param id Summary: Removes a user from the database based on their ID. You may not remove
    ...
    public synchronized User[] getAllUsers() {
        Cursor c = helper.getReadableDatabase().rawQuery("SELECT * FROM " + DBContract.TABLE_USERS, null);
        User[] u = new User[c.getCount()];

        for (int i = 0; i < c.getCount(); i++) {
            c.moveToNext();
            u[i] = new User(c.getString(c.getColumnIndex(DBContract.COL_USER_NAME)), c.getInt(c.getColumnIndex(DBContract.COL_SUPERUSER)), c.getInt(c.getColumnIndex(DBContract.COL_USER_ID))
            );
        }
     ...
     * @param cols
     * @param vals
     * @return Summary: This method gets all the users where the columns in the cols array
     ...
     * @param updateCols
     * @param updateVals
     * @return Summary: Sets the columns in the updateCols array equal to the values in the
     ...
     *
     * @param user
     * @param superuser Summary: This sets whether the passed User parameter is a superuser
     *                  or not.
     *                  ***
     */
     ...
     * @param colors          Summary: Adds a team to the database based on the parameters passed. The
    ...
    public boolean addTeam(Team t) {
        return addTeam(t.getNumber(), t.getName(), t.getSchool(), t.getCity(), t.getRookieYear(), t.getWebsite(), t.getStatePostalCode(), t.getColors());
    }

    public synchronized boolean addTeam(int number, String name, String school, String city, int rookieYear, String website, String statePostalCode, String colors) {
        if (number < 0)
            return false;

        //If there is not already a team with that number...
        if (!hasValue(DBContract.TABLE_TEAMS, DBContract.COL_TEAM_NUMBER, Integer.toString(number))) {
     ...
     * Method: removeTeam
     *
     * @param number Summary: This method removes all information about a team from the database.
     ...
     * @param name
     * @param location
     * @param date     Summary: Adds a competition to the database. Any of these values may be
     ...
     *
     * @param eventID Summary: Deletes a competition and all data associated with it. This includes match data,
     ...
     * @param userID
     * @param comment
     * @param dateStamp  Summary: Adds a new comment to the database. You may leave the Date
     ...
     *
     * @param teamNumber
     * @param dateStamp  Summary: Removes the comment made about the passed team
     ...
     * @param email
     * @param address
     * @param phoneNumber Summary: Adds a contact to the database.
     */
     ...
     *
     * @param id Summary: Removes a contact from the contact list based on ID.
     *           ***
     */
     ...
     * Method: addGame
     *
     * @param name Summary: Adds a new game to the database and creates a unique ID for it.
     ...
     * Method: removeGame
     *
     * @param name Summary: Removes a game from the database based on the id parameter passed.
    ...
    /**
     * **
     * Method: getTeamNumberByRobotID
     * <p/>
     * Description: returns the team number of the specified robot or an empty string
     * if the robot could not be found
     * ***
     */
    public synchronized String getTeamNumberByRobotID(int robotID) {
        SQLiteDatabase db = helper.getReadableDatabase();
        String teamNum = "";
        Cursor c = db.rawQuery("SELECT " + DBContract.COL_TEAM_NUMBER +
                " FROM " + DBContract.TABLE_ROBOTS +
                " WHERE " + DBContract.COL_ROBOT_ID +
                " LIKE " + robotID, null);
        if (c.getCount() > 0) {
            c.moveToFirst();
            teamNum = c.getString(c.getColumnIndex(DBContract.COL_TEAM_NUMBER));
        }
        helper.close();
        return teamNum;
    }
     ...
     * Method: removeRobot
     *
     * @param robotID Summary: Removes a robot from the table based on its ID. Be careful,
     ...
     * @param cols
     * @param vals
     * @return Summary: returns a list of robots created by the cols
     * and vals specified.
     */
     ...
     *
     * @param name        - the name of the new addbutton
     * @param game        - what game this belongs to
     * @param type        - what type of addbutton it is. See DatabaseContract scouting addbutton types
     * @param range       - if the addbutton is a CHOOSER, these are the values that can be picked,
     *                    if the addbutton is a SLIDER, this is the maximum and minimum values.
     * @param description - a short description of the addbutton
     *                    <p/>
     *                    Summary: Adds a new addbutton based off of the parameters passed. If this addbutton is a
     *                    CHOOSER or a SLIDER, the range parameter should not be null.
     * @return True if the addbutton was added to the database correctly. False if there is already
     * a addbutton with this name for this game, this game is not in the database, the type passed
        ...
        for (int key = 0; key < DBContract.COL_KEYS.length; key++) {    //Cycle through
            ...
            if (!c.moveToNext() ||
                ...
                values.put(DBContract.COL_METRIC_KEY, //Assign this key to the new
                        DBContract.COL_KEYS[key]);    //addbutton
     ...
     *
     * @param name
     * @param game Summary: Removes the addbutton from the specified game.
     * @return True if the addbutton was removed. False if this was not a valid game or
     * not a valid addbutton, i.e. it isn't in the database.
     * ***
     */
     ...
     * @param cols
     * @param vals
     * @return Summary: Gets all robot metrics based on the columns and values passed as
     * parameters.
     */
     ...
     * @param description
     * @param displayed
     * @return Summary: Adds a driver addbutton to the database.
     */
     ...
     * <p/>
     * Summary: removes a driver addbutton from the database
     * ***
     */
    ...
    public synchronized boolean removeDriverMetric(int metricID) {
        ...
        if (!hasValue(DBContract.TABLE_DRIVER_METRICS,
            ...
            return false;    //If this is not a real game or addbutton...
        ...
        Cursor c = db.query(DBContract.TABLE_DRIVER_METRICS,
                ...
                new String[]{Integer.toString(metricID)},
                null, null, null);
        c.moveToFirst();//Gets the key for this addbutton
        String key = c.getString(c.getColumnIndex(DBContract.COL_METRIC_KEY));
        String game = c.getString(c.getColumnIndex(DBContract.COL_GAME_NAME));
        ...
        while (c.moveToNext()) {//While there are still more competitions...
            ...
            db.update(DBContract.TABLE_DRIVER_DATA, nullVal,
                    ...
                    new String[]{//Set the addbutton at the key to null
        ...
        db.delete(DBContract.TABLE_DRIVER_METRICS,
                DBContract.COL_METRIC_ID + " LIKE ?",
                new String[]{Integer.toString(metricID)});//Delete the addbutton
     ...
     * @param range
     * @param description Summary: Adds a match performance addbutton to the database based on the parameters
     *                    passed.
     ...
     *
     * @param name
     * @param game Summary: Removes the addbutton based on the name and game parameters passed.
     *             This also removes any data associated with this addbutton, so call carefully.
     * @return True if the addbutton was removed successfully. False if the game
     * given was not a valid game, or the name given was no the name of a addbutton.
     * ***
     */
     ...
     * @param eventID
     * @param robotID
     * @return Summary: Adds the specified robot to the specified event.
     */
     ...
     * @param eventID
     * @param robotID
     * @return Summary: Removes the specified robot from the specified event
     */
     ...
     *
     * @param robotID
     * @return Summary: gets all events out of the database that this robot is in
     */
     ...
     * @param cols
     * @param vals
     * @return Summary: removes match data where cols at n is equal to vals at n
     * ***
     */
     ...
     * @param cols
     * @param vals
     * @return Summary: gets the match data by the columns and values passed in the arrays
     * ***
     */
     ...
     *
     * @param e
     * @param database Summary: gets the averaged and counted data from every robot's match
    ...
    public CompiledData[] getCompiledEventData(int eventID, Query[] querys, SortKey sortKey) {
        ...
        for (int robotCount = 0; robotCount < robots.length; robotCount++) {
            ...
            //Compile the match data for every addbutton
        ...
        if (sortKey != null && compiledData.length > 0) {
            ...
            switch (sortKey.getMetricType()) {
                case SortKey.MATCH_METRIC_TYPE:
                    ...
                    if (metricAdress == -1)
                        break;    //Break if this addbutton is not in the comp data
     ...
     * @param table
     * @param column
     * @param value  Summary: Checks to see if the entered table has the value in the column.
     ...
     *
     * @param e Summary: Sets the event in the scout's event table
     ...
     *
     * @return Summary: Gets all the users in the scout's user table
     */

    public synchronized User[] scoutGetAllUsers() {
        Cursor c = helper.getReadableDatabase().rawQuery("SELECT * FROM " + DBContract.SCOUT_TABLE_USERS, null);
        User[] u = new User[c.getCount()];
     ...
     * @param updateCols
     * @param updateVals
     * @return Summary: updates robots on the scout's database
     */
     ...
     *
     * @param metrics Summary: replaces all the match metrics in the scouting database
     */
     ...
     *
     * @return Summary: gets all of the scout's match data from the database
     */
     ...
     *
     * @param data
     * @return Summary: puts the given match data into the scout's database
     */
    ...
    /**
     * **
     * Method: scoutGetTeamNumberByRobotID
     * <p/>
     * Summary: gets a team number based on the passed robot ID from the scouts robot
     * table
     * ***
     */
    public synchronized String scoutGetTeamNumberByRobotID(int robotID) {
        SQLiteDatabase db = helper.getReadableDatabase();
        String teamNum = "";
        Cursor c = db.rawQuery("SELECT " + DBContract.COL_TEAM_NUMBER +
                " FROM " + DBContract.SCOUT_TABLE_ROBOTS +
                " WHERE " + DBContract.COL_ROBOT_ID +
                " LIKE " + robotID, null);
        if (c.getCount() > 0) {
            c.moveToFirst();
