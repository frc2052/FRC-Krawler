...
class MatchComments(dao: MatchCommentDao, dbManager: DBManager) : AbstractTable<MatchComment, MatchCommentDao>(dao, dbManager) {
    fun insertMatchComment(matchComment: MatchComment): Boolean {
        ...
        matchCommentQueryBuilder.where(MatchCommentDao.Properties.Event_id.eq(matchComment.event_id))
        matchCommentQueryBuilder.where(MatchCommentDao.Properties.Robot_id.eq(matchComment.robot_id))
    ...
    fun query(match_number: Long?, game_type: Int?, robot_id: Long?, event_id: Long?, match_type: Long = MetricHelper.MATCH_GAME_TYPE.toLong()): QueryBuilder<MatchComment> {
